...
def SRPT(fixed,jobs):  #jobs: jobs that haven't been sort  #cmaxFromFixed: the complete time from already fixed jobs 
    # print('jobs\n',jobs)
    jobs=sorted(jobs, key = lambda x: x[2])
    currTime =cmax(fixed) #cmaxFromFixed#cmax(fixed)
    objValue = 0
    jobHeap = MyHeap()
        ...
        # put the most recent job that haven't enter the heap into jobHeap, if needed.
            ...
            if len(jobHeap.minHeap) != 1:  # no need to send job into heap
                ...
                if(jobHeap.minHeap[1][1] == 0):
                    ...
                    # print("Job ", jobHeap.minHeap[1][0]," complete. Delete it from jobHeap.\n\n")
            ...
            # raise Exception('Logic error. Job[0] in jobList has an arriving time that is earlier than currTime.')
