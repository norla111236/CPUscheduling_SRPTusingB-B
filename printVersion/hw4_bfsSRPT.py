...
import heapq as hq
import pyexcel, math, string,sys
...
def bestfs(number):
    ...
    jobHeap=MyHeap()
    for job in jobs:
        # print('test1',job[1]+job[2])
        # print('test',[x for x in jobs if x != job])
        # print('test2',SRPT([x for x in jobs if x != job]))

        # value=job[1]+job[2]+SRPT([x for x in jobs if x != job])
        # value=job[1]+job[2]+SRPT([job],[x for x in jobs if x != job])
    ...
    while len(jobHeap.minHeap)>1:
        # nodeVisited+=1
        # print('\njob queue: ')
        # for i in jobHeap.minHeap:
        #     if i:
        #         i.printInfo()
        print('nodeVisited+=1') 
                ...
                for i in currentJob.toSet:
                    ...
                    for j in newToSet:
                            if i[2]>j[2] and cmaxOfFixed>cmax(currentJob.fixed+[j]):  
                                ...
                                break
                    if skip:
                        print('*** delete branch at ',currentJob.fixed+[j],'***')
                        continue
                    ...
                    if len(copyList)!=1:
                        # newValue=cmax(newFixed)+SRPT(copyList)
                        # newValue=cmax(newFixed)+SRPT(newFixed,copyList)
                        newValue=SRPT(newFixed,copyList)
                    else:
                        # print('testtesttest',newFixed+copyList)
