...
# from hw4_dfsSRPT import dfsForBFS
from sumOfC.SRPT import SRPT
from sumOfC.hw4_dfsSRPT import dfsForBFS
...
deleted=0
updateUB=0
def bestfs(number,switchNumber):
    ...
    while len(jobHeap.minHeap)>1:  #heap index starts from 1
        ...
        elif len(currentJob.fixed)>switchNumber:
            dfsResult=dfsForBFS(currentJob.toSet,currentJob.fixed)
            if dfsResult['value']<upperBound:
                upperBound=dfsResult['value']
                record=dfsResult['record']
                nodeVisited+=dfsResult['nodeVisited']
                updateUB+=dfsResult['updateUB']
        else: 
            if currentJob.value<upperBound:
...
def runBfs(number,switchNumber): #(numberOfJobs, whichLevelToSwitchToDfs)
    print('<<BFS to DFS>>',sep="")
    elapseTime=bestfs(number,switchNumber) 
...
if __name__ == '__main__':
    runBfs(30,16) # params => (numberOfJobs, whichLevelToSwitchToDfs) 